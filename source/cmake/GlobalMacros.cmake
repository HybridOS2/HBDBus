...
# HBDBus), then put it there instead.

macro(HBDBUS_COMPUTE_SOURCES _framework)
...
macro(HBDBUS_INCLUDE_CONFIG_FILES_IF_EXISTS)
...
macro(HBDBUS_ADD_SOURCE_DEPENDENCIES _source _deps)
    set(_tmp)
...
macro(HBDBUS_ADD_PRECOMPILED_HEADER _header _cpp _source)
    if (MSVC)
        ...
        foreach (_src ${_sources})
            HBDBUS_ADD_SOURCE_DEPENDENCIES(${_src} ${PrecompiledBinary})
        endforeach ()
...
macro(HBDBUS_WRAP_SOURCELIST)
    foreach (_file ${ARGN})
        get_filename_component(_basename ${_file} NAME_WE)
        get_filename_component(_path ${_file} PATH)

        if (NOT _file MATCHES "${DERIVED_SOURCES_HBDBUS_DIR}")
            string(REGEX REPLACE "/" "\\\\\\\\" _sourcegroup "${_path}")
            source_group("${_sourcegroup}" FILES ${_file})
        endif ()
    endforeach ()

    source_group("DerivedSources" REGULAR_EXPRESSION "${DERIVED_SOURCES_HBDBUS_DIR}")
endmacro()

macro(HBDBUS_FRAMEWORK_DECLARE _target)
...
macro(HBDBUS_EXECUTABLE_DECLARE _target)
    add_executable(${_target} "${CMAKE_BINARY_DIR}/cmakeconfig.h")
endmacro()

# Private macro for setting the properties of a target.
# Rather than just having _target like HBDBUS_FRAMEWORK and HBDBUS_EXECUTABLE the parameters are
# split into _target_logical_name, which is used for variable expansion, and _target_cmake_name.
# This is done to support HBDBUS_WRAP_EXECUTABLE which uses the _target_logical_name variables
# but requires a different _target_cmake_name.
macro(_HBDBUS_TARGET _target_logical_name _target_cmake_name)
...
macro(_HBDBUS_TARGET_ANALYZE _target)
    if (ClangTidy_EXE)
...
macro(HBDBUS_FRAMEWORK _target)
    _HBDBUS_TARGET(${_target} ${_target})
    _HBDBUS_TARGET_ANALYZE(${_target})
...
# FIXME Move into HBDBUS_FRAMEWORK after all libraries are using this macro
macro(HBDBUS_FRAMEWORK_TARGET _target)
    ...
    add_library(HBDBus::${_target} ALIAS ${_target}_PostBuild)
endmacro()

macro(HBDBUS_EXECUTABLE _target)
    _HBDBUS_TARGET(${_target} ${_target})
    _HBDBUS_TARGET_ANALYZE(${_target})
...
macro(HBDBUS_WRAP_EXECUTABLE _target)
    ...
    _HBDBUS_TARGET(${_target} ${_wrapped_target_name})
    _HBDBUS_TARGET_ANALYZE(${_wrapped_target_name})
...
macro(HBDBUS_CREATE_FORWARDING_HEADER _target_directory _file)
    ...
    # HBDBus2 generate-forwarding-headers script.
...
macro(HBDBUS_CREATE_FORWARDING_HEADERS _framework)
    ...
    if (NOT WIN32)
        ...
        foreach (_currentArg ${ARGN})
            ...
            elseif (_processing_directories)
                ...
                foreach (_file ${_files})
                    HBDBUS_CREATE_FORWARDING_HEADER(${_target_directory} ${_file})
                endforeach ()
            elseif (_processing_files)
                HBDBUS_CREATE_FORWARDING_HEADER(${_target_directory} ${_currentArg})
            endif ()
        endforeach ()
    endif ()
endmacro()

function(HBDBUS_MAKE_FORWARDING_HEADERS framework)
...
function(HBDBUS_COPY_FILES target_name)
    set(options FLATTENED)
...
macro(HBDBUS_DEBUG_DUMP_COMMANDS)
    set(CMAKE_VERBOSE_MAKEFILE ON)
endmacro()

macro(HBDBUS_DEBUG_DUMP_VARIABLES)
...
macro(HBDBUS_ADD_TARGET_PROPERTIES _target _property _flags)
...
macro(HBDBUS_POPULATE_LIBRARY_VERSION library_name)
...
macro(HBDBUS_CREATE_SYMLINK target src dest)
