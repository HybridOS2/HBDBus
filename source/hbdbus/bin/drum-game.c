/*
** drum_game.c -- The code for a drum game by using HBDBus.
**
...
static const char* on_method_get_ball (hbdbus_conn* conn,
...
static int get_ball_from_previous_player (hbdbus_conn* conn, int pn, const char *player_name)
{
    char to_endpoint [HBDBUS_LEN_ENDPOINT_NAME + 1];
...
static void on_game_over (hbdbus_conn* conn,
...
{
    char cmdline_endpoint [HBDBUS_LEN_ENDPOINT_NAME + 1];
    char player_name [HBDBUS_LEN_RUNNER_NAME + 1];
    int cnnfd = -1;
    hbdbus_conn* conn;
    fd_set rfds;
    struct timeval tv;
    ...
    player.running = true;
    player.number = pn;
    hbdbus_conn_set_user_data (conn, &player);

    if (get_ball_from_previous_player (conn, pn, player_name)) {
        goto failed;
    }

    err_code = hbdbus_register_procedure_const (conn, "getBall",
            HBDBUS_LOCALHOST, info->app_name, on_method_get_ball);
    if (err_code) {
        goto failed;
    }

    err_code = hbdbus_subscribe_event (conn, cmdline_endpoint, "GameOver",
            on_game_over);
    if (err_code) {
        goto failed;
    }

    err_code = hbdbus_call_procedure (conn, cmdline_endpoint,
            "notifyReady", player_name,
            HBDBUS_DEF_TIME_EXPECTED, NULL, NULL);
    if (err_code) {
        goto failed;
    }
...
static int fork_a_player (hbdbus_conn* conn, int pn)
{
    pid_t pid;
...
static void term_drum_game (hbdbus_conn* conn)
{
    int err_code;
    struct run_info *info = hbdbus_conn_get_user_data (conn);
...
static char* on_method_notify_ready (hbdbus_conn* conn,
        const char* from_endpoint, const char* to_method,
        const char* method_param, int *err_code)
{
    (void)to_method;
...
int start_drum_game (hbdbus_conn* conn, int nr_players, const char* ball_content)
{
    int err_code;
    struct run_info *info = hbdbus_conn_get_user_data (conn);
