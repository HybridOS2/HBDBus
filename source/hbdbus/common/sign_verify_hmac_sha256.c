        ...
        unsigned char* key, unsigned int key_len)
{
    ...
    size = snprintf (buff, sizeof (buff), HBDBUS_PRIVATE_HMAC_KEY_FILE, app_name, app_name);
    ...
    if (size < HBDBUS_LEN_PRIVATE_HMAC_KEY) {
        fclose (fp);
        return -3;
    }
...
{
    unsigned char key [HBDBUS_LEN_PRIVATE_HMAC_KEY];

    *sig = NULL;
    *sig_len = 0;

    if (read_private_key_for_app (app_name, key, HBDBUS_LEN_PRIVATE_HMAC_KEY)) {
        return HBDBUS_EC_CANT_LOAD;
    }
    ...
    *sig_len = SHA256_DIGEST_SIZE;
    hmac_sha256 (*sig, data, data_len, key, HBDBUS_LEN_PRIVATE_HMAC_KEY);
        ...
        const unsigned char* sig, unsigned int sig_len)
{
    ...
    hmac_sha256 (my_sig, data, data_len, key, HBDBUS_LEN_PRIVATE_HMAC_KEY);
