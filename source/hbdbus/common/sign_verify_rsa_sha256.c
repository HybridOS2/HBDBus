...
static RSA* read_private_key_for_app (const char* app_name)
{
    ...
    size = snprintf (buff, sizeof (buff), HBDBUS_PRIVATE_PEM_KEY_FILE, app_name, app_name);
...
{   
    ...
    size = snprintf (buff, sizeof (buff), HBDBUS_PUBLIC_PEM_KEY_FILE, app_name);
    if (size < 0 || (size_t)size >= sizeof (buff)) {
