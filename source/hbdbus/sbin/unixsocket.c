...
us_handle_accept (USServer* server)
{
    ...
    if (server->nr_clients > MAX_CLIENTS_EACH) {
        ...
        server->on_error (server, (SockClient *)usc, HBDBUS_SC_SERVICE_UNAVAILABLE);
        goto cleanup;
    }
...
int us_handle_reads (USServer* server, USClient* usc)
{
    ...
    if (usc->status & US_WATING_FOR_PAYLOAD) {
        ...
        if (retv > 0) {
            ...
            err_code = retv;
            sta_code = HBDBUS_SC_EXPECTATION_FAILED;
            goto done;
        }
    }
    else {
        ...
        case US_OPCODE_BIN: {
            ...
            if (usc->sz_packet > HBDBUS_MAX_INMEM_PAYLOAD_SIZE ||
                    usc->sz_packet == 0 ||
                    usc->header.sz_payload == 0) {
                err_code = HBDBUS_EC_PROTOCOL;
                sta_code = HBDBUS_SC_PACKET_TOO_LARGE;
                break;
            }
            ...
            if (usc->packet == NULL) {
                ...
                sta_code = HBDBUS_SC_INSUFFICIENT_STORAGE;
                break;
            }
